    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Link
            to="/"
            className="p-2 bg-white rounded-xl shadow-sm border border-gray-100 text-gray-400 hover:text-blue-600 transition-colors"
          >
            <ArrowLeft size={20} />
          </Link>
        ...
        {isLoading ? (
          Array(5)
            ...
            .map((_, i) => (
              <div
                key={i}
                className="bg-white p-6 rounded-3xl border border-gray-100 animate-pulse h-28 w-full"
              ></div>
            ))
          ...
          tripHistory.data.map((trip) => (
            <div
              key={trip.id}
              className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between group active:scale-[0.98] transition-all cursor-pointer"
            >
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-gray-800 line-clamp-1">
                      {trip.fromsite?.site_name || "N/A"} →{" "}
                      {trip.tosite?.site_name || "N/A"}
                    </p>
                  </div>
                </div>
